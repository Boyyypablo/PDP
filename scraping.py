...
def calcular_similaridade(texto, referencias):
    ...
    textos = [texto] + referencias
    vectorizer = TfidfVectorizer()
    
    try:
        X = vectorizer.fit_transform(textos)
        similaridades = cosine_similarity(X[0], X[1:])[0]
        indice_max = similaridades.argmax()
        return similaridades[indice_max], referencias[indice_max]
...
def atualizar_tabela_postgres():
    ...
    print("Coluna 'tematica' atualizada com sucesso!")
